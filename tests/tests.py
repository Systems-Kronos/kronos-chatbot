...
for test_type in test_types:
    ...
    with open(
        f"tests/questions/{test_type}_questions.json", "r", encoding="utf-8"
    ) as file:
    ...
    for item in questions:
        ...
        params = urllib.parse.urlencode(
            {"query": item["message"], "session_id": str(item["session_id"])}
        )
        ...
        try:
            reply_message = json.loads(response_body).get(
    ...
    print(
        f"Todas as perguntas foram enviadas e respostas salvas em '{test_type}_answers.json'."
    )
